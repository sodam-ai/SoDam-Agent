...
- **2026-07-16**: 고객지원팀·PM/제품관리팀·번역/로컬라이제이션팀 추가로 팀 10종 완성. 백업/되돌리기 핵심 파일을 원자적 쓰기로 강화(설치 중 강제종료돼도 반쯤 써진 파일이 안 남음). 보안: 남이 준 팀 파일이 설명 칸에 몰래 권한을 숨겨 넣는 공격(frontmatter 주입)을 찾아 막음(설치 미리보기에 보이는 것과 실제 설치가 항상 같도록 보장).
...
- **2026-06-21**: 마켓플레이스 설치 방식 + 직원 관리 도구(`sodam-agent`) 추가

자세한 내용은 [README](./README.md)의 업데이트 요약 또는 [GitHub 커밋 기록](https://github.com/sodam-ai/SoDam-Agent/commits/)을 참고하세요.
...
### 5단계 · ⭐ 새로 설치한 걸 불러오기 — 가장 중요!
👉 **먼저 이걸 입력창에 치고 Enter** (완전히 껐다 켜지 않아도 바로 반영돼요, 실제 확인됨):
```
/reload-plugins
```
> **왜?** 플러그인은 **켤 때(또는 다시 불러올 때) 한 번** 읽혀요. 그래서 **그 전에는** 방금 깐 직원·명령이 **안 보입니다.** (가장 흔한 막힘 지점이에요!)
> 이래도 안 보이면: 입력창에 `/exit` 치고 나간 뒤 다시 `claude` 로 켜세요(또는 창을 완전히 닫고 다시 열기).
...
| 이런 일이 생기면 | 왜? | 이렇게 하세요 |
|---|---|---|
| 설치했는데 `/agents`·`/sodam-agent:`에 **아무도 안 보여요** | **새로 불러오기(reload)를 안 함** | 먼저 `/reload-plugins` 입력. 그래도 없으면 **Claude Code를 껐다 켜기**(`/exit` → `claude`) |
...
- 직원 지시문·템플릿은 **자체 제작**이며 일부 문구가 **AI 생성**일 수 있어요. AI가 만든 글·이미지는 나라·시점에 따라 저작권이 달라질 수 있으니, **돈 버는 용도(상업적)로 쓰기 전엔 저작권·출처·상업적 사용 가능 여부·비슷한 남의 작품 침해 여부를 직접 확인**하세요. (이 안내는 법률 자문이 아니에요. 헷갈리면 전문가에게 물어보세요.)
