...
<details open>
<summary>📋 <b>Recent major changes</b> (as of 2026-09-11 — click to collapse)</summary>

- **2026-09-11**: Documented that a "connection failed" warning from context7 (an optional tool) while offline or behind a corporate network is harmless. Added a regression test that keeps external dependencies at zero — a lightweight license-safety guard (`01_PRD.md §9`).
- **2026-08-10**: The pre-install diagnostic (`/doctor`) now detects an old Claude Code version that doesn't support plugins before you install. Replaced the now-broken `/agents` guidance with `@`-autocomplete across the install-complete message, all 4 agent-management commands, and all 4 README files. Added a complete-beginner glossary to the README and strengthened the legal section.
- **2026-08-04**: Consolidated the separate beginner guide (4 GUIDE files) into the README — no content lost, less duplicate upkeep. Synced NOTICE's trademark list with the README (added Gemini, Cursor, Context7).
