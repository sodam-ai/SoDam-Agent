...
## 📑 목차
0. [현재 버전 상태 · 업데이트 요약](#0-현재-버전-상태--업데이트-요약)
...
17. [커뮤니티 공유 디렉터리 (선택 기능)](#17-커뮤니티-공유-디렉터리-선택-기능)
- ✨ [Codex에서도 쓰기 (역할 번역 · 베타)](#codex에서도-쓰기-역할-번역--베타)
- ✨ [Gemini CLI에서도 쓰기 (역할 번역 · 베타)](#gemini-cli에서도-쓰기-역할-번역--베타)
- ✨ [Cursor에서도 쓰기 (역할 번역 · 베타)](#cursor에서도-쓰기-역할-번역--베타)
...
- **2026-07-16**: 고객지원팀 · PM/제품관리팀 · 번역/로컬라이제이션팀 추가 — 팀 10종 완성. 백업/되돌리기 핵심 파일의 쓰기 방식을 원자적(tmp→rename)으로 강화(중간에 꺼져도 반쯤 써진 파일이 안 남음). GitHub Actions로 push/PR마다 테스트·보안감사를 자동 실행하는 CI 추가(README 상단 배지로 실시간 확인 가능). **[보안] 남이 준 팀 파일의 설명(description)·모델(model) 값에 줄바꿈을 넣어 에이전트 권한 줄(예: `tools: Bash`)을 몰래 심는 "frontmatter 주입" 공격을 발견·차단**(실제 재현 후 수정, 회귀 테스트 2종 추가 — 설치 미리보기와 실제 설치 내용이 어긋나지 않도록 보장).
...
## Gemini CLI에서도 쓰기 (역할 번역 · 베타)

Codex처럼, **Gemini CLI(Google의 AI 코딩 도구)** 에서도 같은 팀을 쓸 수 있습니다.
Gemini CLI는 Claude Code와 구조가 비슷해서(역할 1개 = 파일 1개) Codex보다 더 가깝게 옮길 수 있지만, **정직하게 말씀드립니다**: 이번 버전은 도구(권한) 제한 없이 **전체 상속**으로 만듭니다 — Gemini의 도구 이름 체계가 Claude Code와 달라(예: `read_file` vs `Read`), 확실한 매핑표 없이 제한하면 오히려 깨질 위험이 있기 때문입니다.

> ⚠️ 이 기능**만**은 **검은 터미널이 필요**합니다(플러그인 설치와 달리). Codex와 마찬가지로 터미널 사용이 익숙한 분께 권합니다.

**준비물**
- 이 저장소를 내려받은 폴더
- **Node.js 18+** (CLI 실행에 필요 — [nodejs.org](https://nodejs.org) LTS)

**설치 방법** — 내려받은 SoDam-Agent 폴더에서, Gemini CLI로 작업할 프로젝트 폴더를 대상으로:
```
node bin/cli.mjs install web-app-team --target gemini --dir "C:\내\프로젝트\폴더"
```
- `--dir` 를 빼면 **현재 폴더**에 만듭니다. · 다른 팀은 `docs-team`·`research-team` 으로 바꿉니다.
- 실행하면 **무엇이 만들어질지 미리보기**를 보여주고 확인을 받습니다(`--yes` 로 확인 건너뛰기 가능).
...
| 파일 | 위치 | 역할 |
|---|---|---|
| `<역할>.md` | `.gemini/agents/<역할>.md` | Gemini CLI가 읽는 역할별 서브에이전트(역할 1개 = 파일 1개, Claude Code와 구조 유사) |
| MCP 설정(mcpServers) | 각 역할 파일 안에 직접 추가 | 자료검색(context7) 같은 도구. **자동으로 안 건드립니다** — 설치 때 나온 스니펫을 원하는 역할 파일에 직접 붙여넣기 |

> 기존 역할 파일이 있으면 **`.bak`으로 백업**한 뒤 새로 씁니다(안전).

**쓰는 법**
- 그 폴더에서 **Gemini CLI를 실행**하면 `.gemini/agents/`의 역할을 서브에이전트로 인식합니다.
- 자료검색(context7)이 필요하면, 설치 때 화면에 나온 **mcpServers 스니펫을 원하는 역할 파일의 frontmatter에 직접 추가**.

**문제 대처**

| 증상 | 해결 |
|---|---|
| Gemini CLI가 역할을 못 찾음 | **그 폴더에서** Gemini CLI를 실행했는지 확인(다른 폴더면 `.gemini/agents/`를 못 읽음) |
| 자료검색 도구가 없음 | mcpServers 스니펫을 원하는 역할 파일에 추가 후 Gemini CLI 재시작 / Node.js 설치 확인 |
| 도구(권한)가 제한 없이 다 열려 있음 | 정상입니다 — 이번 버전은 도구 제한을 만들지 않습니다(베타 한계, 위 설명 참고) |

> 🔎 실제 Gemini CLI 런타임에서의 인식은 사용자 환경에서 한 번 확인해 보시길 권합니다(도구·버전에 따라 다를 수 있음).

---

## Cursor에서도 쓰기 (역할 번역 · 베타)

Codex·Gemini CLI처럼, **Cursor(다른 AI 코딩 에디터)** 에서도 같은 팀을 쓸 수 있습니다.
Cursor의 `.cursor/rules`는 Claude Code 같은 "호출 가능한 역할(서브에이전트)" 개념이 아니라, 조건에 따라 자동/수동으로 컨텍스트에 끼워 넣는 규칙입니다(공식 문서 확인). 그래서 **"똑같은 팀"이 아니라 각 역할을 `AGENTS.md` 안의 "모드" 설명으로 "번역"**해 드립니다(Codex와 같은 방식, 베타).

> ⚠️ 이 기능**만**은 **검은 터미널이 필요**합니다(플러그인 설치와 달리). Codex·Gemini CLI와 마찬가지로 터미널 사용이 익숙한 분께 권합니다.

**준비물**
- 이 저장소를 내려받은 폴더
- **Node.js 18+** (CLI 실행에 필요 — [nodejs.org](https://nodejs.org) LTS)

**설치 방법** — 내려받은 SoDam-Agent 폴더에서, Cursor로 작업할 프로젝트 폴더를 대상으로:
```
node bin/cli.mjs install web-app-team --target cursor --dir "C:\내\프로젝트\폴더"
```
- `--dir` 를 빼면 **현재 폴더**에 만듭니다. · 다른 팀은 `docs-team`·`research-team` 으로 바꿉니다.
- 실행하면 **무엇이 만들어질지 미리보기**를 보여주고 확인을 받습니다(`--yes` 로 확인 건너뛰기 가능).
...
| 파일 | 위치 | 역할 |
|---|---|---|
| `AGENTS.md` | 프로젝트 폴더 루트 | Cursor가 읽는 **역할 설명서**(역할 = "모드") |
| MCP 설정 | `.cursor/mcp.json` | 자료검색(context7) 같은 도구. **Codex·Gemini와 달리 여기는 자동으로 연결됩니다**(확인 후 적용, 기존 설정은 보존) |

> 기존 `AGENTS.md`가 있으면 **`AGENTS.md.bak`으로 백업**한 뒤 새로 씁니다(안전).

**쓰는 법**
- 그 폴더에서 **Cursor를 실행**하면 `AGENTS.md`를 읽고 역할 설명을 컨텍스트로 활용합니다.
- MCP는 설치할 때 이미 `.cursor/mcp.json`에 연결되므로 별도 작업이 필요 없습니다.

**문제 대처**

| 증상 | 해결 |
|---|---|
| Cursor가 역할을 안 따름 | **그 폴더에서** Cursor를 실행했는지 확인(다른 폴더면 `AGENTS.md`를 못 읽음) |
| 자료검색 도구가 없음 | `.cursor/mcp.json`에 context7 항목이 있는지 확인 / Node.js 설치 확인 후 Cursor 재시작 |
| "똑같은 팀"처럼 호출형 서브에이전트가 안 됨 | 정상입니다 — Cursor의 규칙은 호출형이 아니라 컨텍스트 참고용입니다(베타 한계) |

> 🔎 실제 Cursor 런타임에서의 인식은 사용자 환경에서 한 번 확인해 보시길 권합니다(도구·버전에 따라 다를 수 있음).
...
- **이 저장소 구조**:
  - `.claude-plugin/marketplace.json` — 마켓플레이스 카탈로그(8종 모두 등록)
...
| `/doctor`에 `context7 ... skipped` 경고가 뜸 | 여러 팀이 같은 도구(context7)를 각자 들고 있어 하나만 켜짐 | **정상입니다** — 같은 도구가 중복돼 하나만 켠 것으로, 기능에는 영향이 없습니다 |
...
- **프리셋·생성물 출처**: 팀 직원의 지시문과 `sodam-agent`가 만드는 직원 템플릿은 **SoDam-Agent 자체 큐레이션**입니다(타인 저작물 미차용). 다만 일부 문구는 **AI 생성**일 수 있고, AI 생성물의 저작권 지위는 관할·시점에 따라 유동적입니다. 상업적 최종 사용 전 **① 저작권 귀속 ② 출처 ③ 상업적 이용 가능 여부 ④ 유사 저작물 침해 가능성**을 **사용자가 직접 확인**하세요. (본 문서는 법률 자문이 아니며, 확실치 않으면 전문가 검토를 권장합니다.)
...
### 가져온 팀 파일 안전 검증 (신뢰 못 할 입력 전제)
남이 만든 팀 파일(`*.agentroster.json`)을 `import`할 때, 설치 전에 다음을 자동으로 검사합니다:
- **스키마 검증**: 정해진 형식·필드만 허용(알 수 없는 필드·형식 불일치는 설치 중단).
- **크기 제한**: 256KB 초과 파일은 거부(폭주·손상 방지).
- **경로 조작 차단**: 역할·팀 이름은 소문자·숫자·하이픈만 허용(`../` 같은 경로 조작 불가).
- **위험 명령 경고**: 도구(MCP) 명령이 `bash`·`python` 같은 셸/인터프리터면 경고 표시.
- **frontmatter 주입 차단**: 설명(description)·모델(model) 값에 줄바꿈을 넣어 권한 줄을 몰래 심는 공격을 거부(2026-07-16 강화).
- **설치 미리보기 확인**: 무엇이 어디에 써질지 보여주고, 가져온 파일은 `--yes`로도 확인을 건너뛰지 않습니다.
...
## 17. 커뮤니티 공유 디렉터리 (선택 기능)

다른 사용자가 만든 팀 구성을 둘러보거나, 내가 만든 팀을 다른 사람과 나눌 수 있는 **가장 단순한 방식**입니다. **서버·회원가입·로그인이 전혀 없습니다** — 이 GitHub 저장소 안의 카탈로그 파일 하나와 GitHub 자체 Pull Request 기능만 씁니다.

- **둘러보기**: [`community/index.json`](./community/index.json)을 열면 등록된 팀 목록(이름·설명·출처 링크)을 볼 수 있습니다.
- **등록하기**:
  1. `node bin/cli.mjs export <팀이름>`으로 내 팀을 파일(`*.agentroster.json`)로 내보냅니다.
  2. 그 파일을 내 GitHub gist나 저장소에 올립니다(이 저장소엔 팀 파일 원본을 직접 보관하지 않습니다 — 출처 불명 파일이 쌓이는 걸 막기 위함).
  3. `community/index.json`에 이름·설명·내 gist/저장소 링크를 추가하는 **Pull Request**를 보냅니다.
  4. 저장소 관리자가 검토 후 병합합니다.

> ⚠️ 등록된 링크의 파일도 결국 남이 만든 것입니다 — 설치(가져오기) 시 SoDam-Agent이 스키마 검증·위험 명령 경고를 자동으로 하지만, **어떤 팀을 설치할지는 항상 직접 확인**하세요.

현재는 초기 단계라 등록된 팀이 아직 없습니다.
