...
<details open>
<summary>📋 <b>최근 주요 변경</b> (2026-09-11 기준 — 누르면 접기)</summary>

- **2026-09-11**: context7(선택 도구)이 오프라인·사내망 차단 중에 보여줄 수 있는 "연결 실패" 경고가 무해하다는 안내를 문제해결 표에 추가. 외부 패키지 의존성이 계속 0개로 유지되는지 자동 확인하는 회귀 테스트 추가(라이선스 안전장치, `01_PRD.md §9`).
- **2026-08-10**: 설치 전 진단(`/doctor`)이 플러그인 기능을 지원 안 하는 Claude Code 구버전을 미리 감지하도록 개선. 최신 Claude Code에서 무력화된 `/agents` 안내를 `@` 자동완성 방식으로 전면 교체(설치 완료 안내·직원 관리 명령 4종·README 4개 문서 전부). README에 완전 왕초보용 용어표를 추가하고 법률 고지를 강화.
- **2026-08-04**: 별도로 있던 왕초보 가이드(GUIDE) 문서 4개를 README 하나로 통합(내용 손실 없음, 중복 관리 부담 축소). NOTICE의 상표 목록을 README와 동기화(Gemini·Cursor·Context7 추가).
...
> 💡 **컴퓨터·스마트폰·AI가 정말 처음이신가요? 여기부터 읽어보세요.**
> 이 문서는 여러분이 **Claude Code라는 프로그램을 이미 열 수 있는 상태**라고 가정합니다. (Claude Code 자체를 컴퓨터에 설치하는 방법은 이 문서가 다루는 범위가 아니며, [공식 사이트](https://claude.com/claude-code)에 별도로 안내되어 있습니다 — 그건 이미 되어 있다고 보고, 그 다음부터를 이 문서가 설명합니다.) 그 다음부터 필요한 낱말은 이게 전부입니다 — 급하지 않게 천천히 읽어보세요.
>
> | 낱말 | 쉬운 뜻 |
> |---|---|
> | **클릭(click)** | 마우스 버튼을 한 번 눌렀다 떼는 동작. 스마트폰이면 손가락으로 화면을 한 번 톡 누르는 "탭"과 같은 동작입니다. |
> | **입력창** | 글자를 타이핑해서 넣는 네모난 칸. Claude Code 화면 아래쪽에 있습니다(아래에서 설명하는 "터미널"과는 다릅니다). |
> | **복사 / 붙여넣기** | 글자를 그대로 옮기는 것. 옮기고 싶은 글자를 마우스로 드래그해 파랗게 선택한 뒤 키보드에서 `Ctrl`키를 누른 채로 `C`(복사) → 붙일 자리를 클릭하고 `Ctrl`을 누른 채로 `V`(붙여넣기). |
> | **폴더(folder)** | 컴퓨터 안에서 파일을 담아두는 "서랍"이나 "상자" 같은 것. |
> | **파일(file)** | 문서·사진처럼 컴퓨터에 저장된 하나하나의 항목. |
> | **다운로드(download)** | 인터넷에 있는 것을 내 컴퓨터로 가져와 저장하는 것. |
> | **프로그램(앱)** | 컴퓨터·스마트폰에 설치해서 실행하는 도구. Claude Code도 이런 프로그램의 하나입니다. |
> | **터미널(=검은 창, 명령창)** | 글자로 명령을 입력하는 검은 화면. **이 문서에서 추천하는 방법(방법 A)은 터미널이 전혀 필요 없습니다** — Claude Code 안의 "입력창"만 쓰면 됩니다. 터미널은 아래 Codex·Gemini CLI·Cursor 같은 **선택 기능**에서만 예외적으로 등장합니다. |
> | **플러그인** | 프로그램에 *끼워서 기능을 추가하는 부품*입니다. (SoDam-Agent가 바로 이 부품입니다.) |
> | **마켓플레이스** | 플러그인을 *모아 둔 가게*입니다. 여기에 등록해야 플러그인을 꺼내 쓸 수 있습니다. |
> | **에이전트(서브에이전트)** | AI 직원 한 명을 가리키는 말입니다. |
> | **AI(인공지능) · 챗봇** | 사람이 글자로 질문하면, 사람처럼 글자로 답해주는 프로그램입니다. Claude Code는 "Claude"라는 AI와 대화하는 프로그램입니다. |
>
> 이 낱말들의 뜻만 알면 아래부터는 순서대로 따라 하기만 하면 됩니다. 중간에 막히면 언제든 [§11 문제·오류 대처](#11-문제--오류-대처-증상--원인--해결)에서 같은 증상을 찾아보세요.
...
5. **새로 설치한 걸 불러옵니다.**
   ...
   - ⚠️ **가장 흔한 막힘 지점**: 설치 직후엔 입력창에 `@`·`/sodam-agent:`를 입력해도 자동완성에 **안 보일 수 있습니다.** 플러그인은 **켤 때(또는 재로드할 때) 한 번 로드**되기 때문입니다. `/reload-plugins`로도 안 보이면, Claude Code를 **완전히 껐다 켜기**(재시작)로 넘어가세요.
...
3. 입력창에 `@web-app-team:` 을 입력해 자동완성으로 **`web-app-team:reviewer`** 같은 이름이 보이면 성공 → 바로 `web-app-team:reviewer 서브에이전트로 이 코드 검토해줘` 처럼 사용.
...
- **확인**: 입력창에 `@web-app-team:` 처럼 입력 → 자동완성 목록에서 `web-app-team:reviewer` 등이 보이는지 확인.
  - 이름에 `팀:` 이 붙어 있어, 다른 데서 온 비슷한 이름과 **확실히 구분**됩니다.
- **부르는 법(셋 중 편한 것):**
  1. 자연어: `web-app-team:reviewer 한테 이 코드 검토 맡겨줘`
  2. 직접 지정(@-mention): 입력창에 `@` 입력 → 자동완성 목록에서 선택.
...
`sodam-agent` 도구를 설치하면, **검은 터미널 없이 Claude Code 안에서** 슬래시 명령으로 직원을 직접 만들고 관리할 수 있습니다. (Claude Code 최신 버전은 `/agents`에 직원을 직접 만드는 화면조차 없어 파일을 손으로 고치라고 안내합니다 — 그 대신 **명령 한 줄**로 쉽게 만들 수 있게 한 것.)
...
| `@<이름>` (입력창에 직접 입력) | 설치된 AI 직원 자동완성으로 확인·호출 (최신 Claude Code는 `/agents`가 목록을 보여주지 않음) |
...
| 증상 | 원인 | 해결 |
|---|---|---|
| 설치했는데 `@`·`/sodam-agent:`를 입력해도 자동완성에 **아무것도 안 보임** | **새로 불러오기(reload)를 안 함** (플러그인은 켤 때·재로드 시 로드) | 먼저 `/reload-plugins` 입력. 그래도 없으면 **Claude Code를 껐다 켜기** |
...
| 마켓은 등록됐는데 `@팀이름:`을 입력해도 자동완성에 직원이 **0명** | `marketplace add`만 하고 **`install`을 안 함** | `/plugin install <팀>@sodamagent-marketplace` 를 **따로** 실행 (등록 ≠ 설치) |
...
| context7 연결에 "실패" 경고(오프라인·회사망 차단 중에도) | context7은 선택(optional) 도구인데 항상 연결을 시도하도록 되어 있음(알려진 사소한 결함) | **무시해도 됩니다** — 핵심 기능엔 영향 없음, 인터넷이 연결되면 자동으로 정상 작동합니다 |
...
> ⚠️ **먼저 읽어주세요 — 법률 자문이 아닙니다.** 이 섹션은 이해를 돕기 위한 요약일 뿐, **변호사·법무 전문가의 자문을 대신하지 않습니다.** 사업·판매·고객사 납품 등 **상업적 목적**으로 쓰려는 경우, 아래 내용을 참고 정보로만 삼고 **반드시 전문가 검토**를 받으세요. 이 도구는 **"있는 그대로(AS IS)" 제공되며, 어떠한 형태의 보증도 하지 않습니다.**
...
- **라이선스: Apache License 2.0.** 상업적 사용·수정·복제·재배포가 **허용**됩니다.
  ...
  - **책임 제한(쉬운 설명)**: Apache-2.0 §8에 따라, 이 도구를 쓰다가 발생한 손해에 대해 저작자·기여자는 **법이 허용하는 한도 내에서 책임지지 않습니다.** 정확한 법적 문구는 [`LICENSE`](./LICENSE) 원문이 기준입니다(이 README의 설명이 아님).
...
- **상업적 사용 범위**: Apache-2.0에 따라 **수정·복제·포크·재배포·판매·서비스 운영·교육 자료·회사/고객사 납품**이 대부분 허용됩니다. 단 **아래는 전부 사용자 책임**입니다.
...
- **프리셋·생성물 출처**: 팀 직원의 지시문과 `sodam-agent`가 만드는 직원 템플릿은 **SoDam-Agent 자체 큐레이션**입니다(타인 저작물 미차용). 다만 일부 문구는 **AI 생성**일 수 있고, AI 생성물의 저작권 지위는 관할·시점에 따라 유동적입니다.

**✅ 상업적으로 쓰기 전 최종 체크리스트** (전부 사용자가 직접 확인할 항목 — SoDam-Agent가 대신 보증하지 않습니다)
- [ ] `LICENSE`·`NOTICE` 원문을 읽고 조건(고지 유지·변경 표시·상표 미부여)을 이해했다.
- [ ] 연결하는 MCP(context7/Upstash 등)·외부 API의 요금제·이용약관을 해당 서비스 사이트에서 직접 확인했다.
- [ ] AI 생성 콘텐츠(프리셋 문구 등)의 저작권 귀속·출처·유사 저작물 침해 가능성을 확인했다(관할·시점에 따라 다름).
- [ ] "SoDam-Agent"라는 이름이 내가 속한 지역·업종에서 상표 충돌이 없는지 확인했다.
- [ ] 확신이 서지 않는 항목이 있다면 **법률 전문가에게 검토**를 요청했다.
